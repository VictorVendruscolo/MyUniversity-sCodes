...
//   * Colocacao interativa em TRES cliques, com pre-visualizacao em tempo real:
//       1o clique -> fixa o VERTICE (apice);
//       move o mouse -> o vetor apice->mouse define a ALTURA (distancia) e a DIRECAO;
//       2o clique -> fixa altura e direcao;
//       move o mouse -> a distancia do 2o ponto ao mouse alarga/afina o RAIO da base;
//       3o clique -> fixa o raio e salva o cone.
...
//
// ----------------------------------------------------------------------------------
// PROCEDENCIA DE 2 RECURSOS QUE NAO ESTAO NO canvas.cpp BASE (para a avaliacao)
// ----------------------------------------------------------------------------------
...
// Apenas dois recursos nao aparecem no canvas.cpp base, e estao justificados:
//   * atan2 (na funcao drawCone): trigonometria de transformacoes vista no CAP.4 do
//     Sumanta Guha; usada so para achar a direcao do cone (angulo do vetor mouse).
//   * glutPassiveMotionFunc (no main): a versao "passiva" (mouse sem botao) do
...
static int pointCount = 0; // Quantidade de pontos ja informados (0, 1 ou 2 - o cone usa 3 cliques).
static int tempX, tempY; // Coordenadas do primeiro ponto clicado (apice do cone).
static int temp2X, temp2Y; // [T1-NOVO] Segundo ponto do cone (fixa altura+direcao no 2o clique).
...
//
// O cone e criado em TRES cliques (ver mouseControl):
//   (x1, y1) = VERTICE (apice)   -> 1o clique;
//   (x2, y2) = 2o ponto          -> fixa a ALTURA (distancia ate o apice, via sqrt) e a
//              DIRECAO (angulo do vetor apice->2o ponto, via atan2 - trig. do Cap.4 do
//              Sumanta Guha);
//   baseRadius = RAIO da base    -> definido no 3o clique pela distancia do 2o ponto ao
//              mouse (o usuario alarga/afina a base arrastando).
...
public:
   ...
   {
	  x1 = x1Val; y1 = y1Val; x2 = x2Val; y2 = y2Val;
	  baseRadius = radiusVal;                   // [T1] Raio escolhido no 3o clique.
...
void Cone::drawCone()
{
   // O vetor do vertice (x1,y1) ate o 2o ponto (x2,y2) define a ALTURA (distancia) e a
   // DIRECAO (angulo) do cone. O RAIO da base vem separado, escolhido no 3o clique.
   float dx = (float)(x2 - x1);
   float dy = (float)(y2 - y1);
   float height = sqrt(dx * dx + dy * dy);      // Altura = distancia vertice -> 2o ponto (sqrt, como em Cap3/MouseMotion.cpp).
   float radius = baseRadius;                   // Raio da base definido interativamente (3o clique).
...
// esta sendo colocada (substitui o antigo "drawTempPoint" do original).
// (ITEM I) Mostra em VERMELHO a primitiva do 1o ponto (tempX,tempY) ate o mouse
// (curX,curY), antes do clique final. Para o cone ha duas etapas: pointCount == 1
// (definindo altura+direcao) e pointCount == 2 (definindo o raio da base).
...
void drawScene(void)
{
   ...
   // Linha/retangulo usam so pointCount == 1; o cone pode estar em 1 (altura+direcao) ou 2 (raio).
...
void mouseControl(int button, int state, int x, int y)
{
   if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN)
   {
      ...
      else
	  {
         ...
         else if (primitive == CONE)     // [T1-NOVO] Colocacao do cone em TRES cliques.
		 {
	        if (pointCount == 0)
			{
			...
			{
               temp2X = x; temp2Y = y;  // 2o clique: fixa altura + direcao (2o ponto).
               curX = x; curY = y;
		       pointCount++;
			}
	        else                         // pointCount == 2
...
// E chamada continuamente enquanto o mouse se move. Enquanto a primitiva esta em
// construcao (pointCount == 1, ou 2 no caso do cone), atualiza a posicao atual do mouse
// e pede redesenho, produzindo a pre-visualizacao em tempo real.
...
void printInteraction(void)
{
   ...
   cout << "Left click on a box on the left to select a primitive." << endl
        << "Point: one click. Line/rectangle: two clicks. Cone: three clicks" << endl // [T1]
        << "(1st = apex, 2nd = height+direction, 3rd = base radius)." << endl // [T1]
