...
- `pointCount` é uma **máquina de estados** de cliques: `0` = esperando 1º ponto,
  `1` = tem o 1º ponto e espera o 2º. O **cone** usa um estado a mais (`2`), porque tem
  **3 cliques** (ápice → altura/direção → raio). A pré-visualização aparece nos estados 1 e 2.
...
### 3.1 A classe `Cone` (dois pontos + um raio; 3 cliques)
```c
class Cone {
  Cone(int x1,y1,x2,y2, float radius);  // (x1,y1)=vértice; (x2,y2)=2º ponto; radius=base
  void drawCone();
...
private:
  int x1,y1,x2,y2;
  float baseRadius;              // raio da base (3º clique)
  float angleX,angleY,angleZ,scale;
};
```
- Guarda **dois pontos** (como `Line`/`Rectangle`) **+ um raio** próprio (`baseRadius`).
- Do vetor `(x1,y1)→(x2,y2)` saem a **altura** (distância, `sqrt`) e a **direção** (ângulo,
  `atan2`). O **raio** é independente — escolhido no 3º clique.
- Guarda também o **estado de transformação**: 3 ângulos + escala.
- `translate` move `(x1,y1)` **e** `(x2,y2)` juntos → o cone inteiro anda sem deformar.

> **Pergunta típica:** "Por que 3 cliques?" → 1º = ápice; movendo o mouse define altura e
> direção; 2º fixa altura+direção; movendo o mouse a distância ao 2º ponto vira o raio; 3º
> fixa o raio. Assim o usuário escolhe **também a largura da base**, de forma interativa.
...
float radius   = baseRadius;               // (B)  <- escolhido no 3º clique
...
- **(B) `radius = baseRadius`** — o raio da base **escolhido pelo usuário no 3º clique**
  (a distância do 2º ponto ao mouse). É o que deixa o cone mais largo ou mais fino.
  *(Na pré-visualização da etapa 1, quando o raio ainda não foi dado, usa-se um provisório
  de `0.40*altura` só para o preview parecer um cone.)*
...
2. **`mouseMotion(x,y)`**: callback chamado a cada movimento. Enquanto a primitiva está em
   construção (`pointCount == 1`, ou `2` no cone), atualiza `curX,curY` (convertendo
   `y = height - y`) e chama `glutPostRedisplay()` → a tela redesenha.
3. **`drawPreview()`** dentro do `drawScene`: desenha em **vermelho** a primitiva em
   construção. **O cone tem 2 etapas**: com `pointCount == 1` monta um cone `temp` usando o
   mouse como 2º ponto (altura+direção); com `pointCount == 2` a altura/direção já estão
   fixas e o raio vem da distância do 2º ponto ao mouse.

**Por que `glutPassiveMotionFunc` E `glutMotionFunc`?**
- `glutMotionFunc` só dispara com o **botão pressionado**. Mas entre os cliques o
...
### 3.6 Colocação do cone (três cliques) — dentro do `mouseControl`
- 1º clique (`pointCount == 0`): guarda `tempX,tempY` (vértice); `pointCount = 1`.
- Move o mouse → preview da **altura + direção** (mouse como 2º ponto).
- 2º clique (`pointCount == 1`): guarda `temp2X,temp2Y` (fixa altura+direção); `pointCount = 2`.
- Move o mouse → preview do **raio** (distância de `temp2` ao mouse).
- 3º clique (`pointCount == 2`): calcula `r = sqrt(...)` e faz
  `cones.push_back(Cone(tempX,tempY,temp2X,temp2Y,r))`; zera `pointCount`.
- É o **mesmo esquema** de máquina de estados do canvas, só que com um estado a mais.
...
3. **Como o cone é criado (os 3 cliques)?** 1º clique = **ápice**; movendo o mouse, o vetor
   ápice→mouse dá a **altura** (`sqrt`) e a **direção** (`atan2`). 2º clique fixa altura e
   direção; movendo o mouse, a distância do 2º ponto ao mouse dá o **raio da base** (`sqrt`).
   3º clique fixa o raio e salva. Assim o usuário escolhe interativamente **altura, direção
   e largura** da base.
...
6. **Por que dois callbacks de movimento do mouse?** `glutMotionFunc` só dispara com botão
   pressionado; entre os cliques o botão está solto, então é preciso o
...
9. **De onde vem o raio da base?** Do **3º clique** (`baseRadius`), pela distância do 2º
   ponto ao mouse. Na pré-visualização da 1ª etapa, quando o raio ainda não existe, usa-se
   um provisório de `0.40*altura` só para o preview parecer um cone.
...
2. **Cubra o `drawCone` com a mão** e tente reescrever de memória as etapas (A)–(J).
...
> Dica: se você consegue explicar **por que** cada uma das etapas (A)–(J) do `drawCone`
